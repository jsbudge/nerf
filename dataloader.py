...
class NeRFModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        self.train_dataset = get_dataset(dataset_name=self.config.dataset_name, base_dir=self.config.base_dir,
                                         split="train", factor=self.config.factor, device=self.config.device)

        self.val_dataset = get_dataset(dataset_name=self.config.dataset_name, base_dir=self.config.base_dir,
                                         split="test", factor=self.config.factor, device=self.config.device)


    def train_dataloader(self) -> DataLoader:
        loader = DataLoader(self.train_dataset, batch_size=self.config.batch_size, shuffle=True)
        loader.h = self.train_dataset.h
        loader.w = self.train_dataset.w
        loader.near = self.train_dataset.near
        loader.far = self.train_dataset.far
        return loader

    def val_dataloader(self) -> Union[DataLoader, List[DataLoader]]:
        loader = DataLoader(self.val_dataset, batch_size=self.config.batch_size, shuffle=False)
        loader.h = self.train_dataset.h
        loader.w = self.train_dataset.w
        loader.near = self.train_dataset.near
        loader.far = self.train_dataset.far
    ...
    def render_loader(self) -> DataLoader:
        render_dataset = get_dataset(dataset_name=self.config.dataset_name, base_dir=self.config.base_dir,
                                         split="render", factor=self.config.factor, device=self.config.device)
        loader = DataLoader(render_dataset, batch_size=render_dataset.h * render_dataset.w, shuffle=False)
        loader.h = render_dataset.h
        loader.w = render_dataset.w
        loader.near = render_dataset.near
        loader.far = render_dataset.far
        return loader
