...
class SARNeRF(LightningModule):
    ...
    def forward(self, rays_o, rays_d, rays_p, mfilt, radii, near, far, nsam):
        comp_rgbs = []
        distances = []
        accs = []
        for l in range(self.num_levels):
            ...
            new_encodings = self.density_net1(new_encodings)
            raw_density = self.final_density(new_encodings).reshape((-1, self.num_samples, 1))
            raw_norms = self.final_norm(new_encodings).reshape((-1, self.num_samples, 3))
            ...
            # volumetric rendering
            raw_density = self.density_activation(raw_density + self.density_bias)
            comp_rp, distance, acc, weights, alpha = volumetric_scattering(raw_norms.reshape((-1, radii.shape[1], self.num_samples, 3)),
                                                                           raw_density.reshape((-1, radii.shape[1], self.num_samples)),
                                                                           rays_p,
                                                                           t_vals.reshape((-1, radii.shape[1], self.num_samples + 1)), rays_d,
                                                                           torch.linspace(near[0, 0], far[0, 0], nsam),
                                                                           self.wavelength)
            ...
            accs.append(acc)
        if self.return_raw:
            raws = torch.cat((torch.clone(raw_norms).detach(), torch.clone(raw_density).detach()), -1).cpu()
            # Predicted RGB values for rays, Disparity map (inverse of depth), Accumulated opacity (alpha) along a ray
            return torch.stack(comp_rgbs), torch.stack(distances), torch.stack(accs), raws
        else:
            # Predicted RGB values for rays, Disparity map (inverse of depth), Accumulated opacity (alpha) along a ray
    ...
    def train_val_get(self, batch, batch_idx, kind='train'):
        rays_o, rays_d, rays_p, mfilt, radii, near, far, target = batch

        # Generate rays for random sampling
        rgb, disp, acc = self.forward(rays_o, rays_d, rays_p, mfilt, radii, near, far, target.shape[1])
